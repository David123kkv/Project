...
"""
Designed by Qiao
相关博客：https://blog.csdn.net/qq_62902390?spm=1010.2135.3001.5343
https://mp.csdn.net/mp_blog/creation/editor/130274059
"""
...
class Emotion_MainWindow(Ui_MainWindow):
    ...
    def button_open_camera_click(self):
        ...
        self.textEdit_camera.setText('Live cam is turned off')
        self.textEdit_video.setText("Video is unchecked")
        ...
        if self.timer_camera.isActive() == False:  # 检查定时状态
            ...
            if flag == False:  # 相机打开失败提示
                msg = QtWidgets.QMessageBox.warning(self.centralwidget, u"Warning",
                                                    u"Please check whether the camera is connected to the computer correctly! ",
                                                    buttons=QtWidgets.QMessageBox.Ok,
                                                    defaultButton=QtWidgets.QMessageBox.Ok)

            else:
                # 准备运行识别程序
                self.textEdit_pic.setText('File is unchecked')
                QtWidgets.QApplication.processEvents()
                self.textEdit_camera.setText('Live camera is turned on')
                self.label_face.setText('Activating the identification system...\n\nleading')
                # 新建对象
            ...
            self.textEdit_camera.setText('Live cam is turned off')
            self.textEdit_pic.setText('File is unchecked')
            self.textEdit_video.setText('File is unchecked')
            # gif = QMovie(':/newPrefix/icons/scan.gif')
            # self.label_face.setMovie(gif)
            # gif.start()
            self.label_outputResult.clear()
    ...
    def button_open_video_click(self):
        ...
        if self.timer_video.isActive() == False:  # 检查定时状态
            ...
            fileName_choose, filetype = QFileDialog.getOpenFileName(
                self.centralwidget, "Choose a picture file",
                self.path,  # 起始路径
                "Video(*.mp4;)")  # 文件类型
            self.path = fileName_choose  # 保存路径
            if fileName_choose != '':
                self.textEdit_video.setText(fileName_choose + 'The file is selected')
                # 新建对象
    ...
    def choose_pic(self):
        ...
        self.path = fileName_choose  # 保存路径
        if fileName_choose != '':
            self.textEdit_pic.setText(fileName_choose + 'The file is selected')
            self.label_face.setText('Activating the identification system...\n\nleading')
            QtWidgets.QApplication.processEvents()
            # 生成模型对象
            self.emotion_model = Emotion_Rec(self.model_path)
    ...
    def choose_model(self):
        ...
        fileName_choose, filetype = QFileDialog.getOpenFileName(self.centralwidget,
                                                                "Choose a picture file", getcwd(),  # 起始路径
                                                                "Model File (*.hdf5)")  # 文件类型
        # 显示提示信息
        if fileName_choose != '':
            self.model_path = fileName_choose
            self.textEdit_model.setText(fileName_choose + ' Selected')
        else:
            self.textEdit_model.setText('Use the default model')
